...
class UserController extends AbstractController
{
    ...
    public function modifPwd(Request $request, User $user, UserPasswordHasherInterface $hasher, UserRepository $userRepository): Response
    {
        ...
        if ($userConnect->getUserIdentifier() != $user->getEmail()) {
        ...
        {
            if ($hasher->isPasswordValid($user, $request->request->get("pwdOld"))) {
                if ($request->request->get("pwdNew") === $request->request->get("pwdNewDuplique")) {
                    $user->setPassword($hasher->hashPassword($user, $request->request->get("pwdNew")));
                    $userRepository->save($user, true);
